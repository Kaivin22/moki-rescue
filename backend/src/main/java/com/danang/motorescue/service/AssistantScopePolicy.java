...
public class AssistantScopePolicy {
    ...
    private static final Pattern ABOUT_APP = Pattern.compile(
            "^(moki rescue|moki|ung dung moki rescue|moki rescue app|motorescue|ung dung motorescue|motorescue app) " +
            "(la gi|ho tro gi|lam duoc gi|" +
    ...
    private static final Pattern GENERIC_APP_HELP = Pattern.compile(
            "\\b(cach dung|huong dan|tro giup|help|how to use|use|using)\\b.{0,40}\\b(ung dung|app|moki rescue|moki|motorescue)\\b|" +
            "\\b(ung dung|app|moki rescue|moki|motorescue)\\b.{0,40}\\b(cach dung|huong dan|tro giup|help|how|use|using)\\b");
    ...
    public Decision classify(String input) {
        ...
        if (EMERGENCY.matcher(normalized).find()) {
            return new Decision(Disposition.EMERGENCY,
                    english
                            ? "If someone is injured or there is fire or a fuel leak, move away from danger and call the appropriate 113/114/115 service first. Moki Rescue does not replace emergency services."
                            : "Nếu có người bị thương, cháy hoặc rò nhiên liệu, hãy rời khỏi vị trí nguy hiểm và gọi 113/114/115 phù hợp trước. Moki Rescue không thay thế dịch vụ khẩn cấp.");
        }
        if (GREETING.matcher(normalized).matches()) {
            return new Decision(Disposition.GREETING,
                    english
                            ? "I can help with rescue requests, status tracking, quotes, maps, accounts and other Moki Rescue features."
                            : "Tôi có thể hướng dẫn bạn tạo yêu cầu cứu hộ, theo dõi trạng thái, báo giá, bản đồ, tài khoản và các tính năng của Moki Rescue.");
        }
        ...
        if (DIAGNOSIS.matcher(normalized).find()) {
            return new Decision(Disposition.DIAGNOSIS,
                    english
                            ? "I cannot diagnose a vehicle or guide self-repair. Create a Moki Rescue request, describe only what you observe, and wait for a rescuer to inspect it safely."
                            : "Tôi không chẩn đoán hoặc hướng dẫn tự sửa xe. Bạn có thể tạo yêu cầu Moki Rescue, mô tả dấu hiệu quan sát được và chờ cứu hộ viên kiểm tra an toàn.");
        }
    ...
    private static Decision outOfScope(boolean english) {
        return new Decision(Disposition.OUT_OF_SCOPE,
                english
                        ? "I only answer questions about the Moki Rescue app and its in-app rescue process."
                        : "Tôi chỉ hỗ trợ các câu hỏi về ứng dụng Moki Rescue và quy trình cứu hộ trong ứng dụng.");
    }
